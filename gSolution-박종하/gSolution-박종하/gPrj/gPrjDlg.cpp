...
BEGIN_MESSAGE_MAP(CgPrjDlg, CDialogEx)
	...
	ON_EN_CHANGE(IDC_SIZE_EDIT, &CgPrjDlg::OnEnChangeSizeEdit)
END_MESSAGE_MAP()
...
void CgPrjDlg::OnBnClickedBtnGetData()
{
	...
	// 무게 중심을 출력
	cout << "무게 중심 좌표: " << dCenterX << ", " << dCenterY << endl;

	// TODO: 중심 좌표(dCenterX, dCenterY)를 사용하여 원의 중심을 찾는 코드 추가
	// 여기서는 간단하게 원의 중심을 무게 중심으로 설정

	// 사용자가 입력한 크기에 따라 원의 중심을 설정
	int nSize = GetDlgItemInt(IDC_SIZE_EDIT);
	double dCircleCenterX = dCenterX;
	double dCircleCenterY = dCenterY;

	// 무게 중심에서 원의 지름에 따라 이동
	dCircleCenterX += nSize / 2.0;
	dCircleCenterY += nSize / 2.0;

	// 빨간색으로 중심 표시
	CDC* pDC = m_pDlgImage->GetDC();
	pDC->SetPixel((int)dCenterX, (int)dCenterY, RGB(255, 0, 0));  // 무게 중심에 빨간색 점 찍기

	// 파란색으로 원의 중심 표시
	pDC->SetPixel((int)dCircleCenterX, (int)dCircleCenterY, RGB(0, 0, 255));  // 원의 중심에 파란색 점 찍기

	// 중심 좌표 콘솔에 출력
	cout << "원의 중심 좌표: " << dCircleCenterX << ", " << dCircleCenterY << endl;
...
void CgPrjDlg::OnEnChangeSizeEdit()
{
	// 에디트 컨트롤에서 텍스트를 가져오기
	CString strSize;
	GetDlgItemText(IDC_SIZE_EDIT, strSize);

	// 텍스트를 정수로 변환
	int nSize = _ttoi(strSize);

	// 텍스트가 변경되었을 때, m_pDlgImage의 DrawCircle 함수 호출
	if (m_pDlgImage) {
		...
		// 크기 설정
		m_pDlgImage->SetCircleSize(nSize);
